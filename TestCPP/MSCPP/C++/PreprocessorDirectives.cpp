...
namespace PreprocessorDirectives
{
	...
	void macro_definitions()
	{
		/*
			...
			it replaces any occurrence of identifier in the rest of the code by replacement.
			...
			but also replacing each argument by its identifier,
		...
		*/
		...
		/*
			...
			macro definitions can be a tricky feature.
			But, be careful: code that relies heavily on complicated macros become less readable,
			since the syntax expected is on many occasions different from the normal expressions programmers expect in C++.
		*/
	}
	...
	void conditional_inclusions()
	{
		/*
			...
			In this case, the line of code int table[TABLE_SIZE]; is only compiled if TABLE_SIZE was previously defined with #define,
			independently of its value.
			If it was not defined, that line will not be included in the program compilation.
			#ifndef serves for the exact opposite:
			the code between #ifndef and #endif directives is only compiled if the specified identifier has not been previously defined.
			For example:
			...
			In this case, if when arriving at this piece of code, the TABLE_SIZE macro has not been defined yet,
			it would be defined to a value of 100.
			...
			for the portion of code they surround to be compiled.
	...
	void source_file_inclusion()
	{
		/*
			...
			This is used to include headers provided by the implementation,
			...
			The syntax used in the second #include uses quotes, and includes a file.
			The file is searched for in an implementation-defined manner,
			which generally includes the current path. In the case that the file is not found,
			the compiler interprets the directive as a header inclusion,
			just as if the quotes ("") were replaced by angle-brackets (<>).
		*/
		{
			system("pause");
		}
	...
	void pragma_directive()
	{
		/*
			...
			Consult the manual or the reference of your compiler for more information on the possible parameters
			that you can define with #pragma.
		...
		/*
			...
			The following macro names are always defined (they all begin and end with two underscore characters, _):
				...
				__cplusplus			An integer value. All C++ compilers have this constant defined to some value.
									...
									Note that many compilers are not fully conforming
									and thus will have this constant defined as neither of the values above.
