...
namespace Constants
{
	void literals()
	{
		/*
			...
			We have already used some in previous chapters to give specific values to variables
			or to express messages we wanted our programs to print out,
			...
			And for hexadecimal, they are preceded by the characters 0x (zero, x).
			...
			These literal constants have a type, just like variables.
			...
			They can include either a decimal point, an e character (that expresses "by ten at the Xth height",
			where X is an integer value that follows the e character),
			...
			The default type for floating-point literals is double. Floating-point literals of type float
			or long double can be specified by adding one of the following suffixes:
			...
			or uppercase letters with no difference in meaning.
			...
			The first two expressions represent single-character literals,
			and the following two represent string literals composed of several characters.
			...
			Both single-character and string literals require quotation marks surrounding them to distinguish them from possible variable identifiers
			...
			whereas 'x' (enclosed within single quotation marks) would refer to the character literal 'x'
			...
			or impossible to express otherwise in the source code of a program,
			like newline (\n) or tab (\t).
			...
			Internally, computers represent characters as numerical codes: most typically,
			they use one extension of the ASCII character encoding system (see ASCII code for more info).
			Characters can also be represented in literals using its numerical code by writing a backslash character (\) followed by the code expressed as an octal (base-8)
			or hexadecimal (base-16) number. For an octal value,
			the backslash is followed directly by the digits; while for hexadecimal,
			an x character is inserted between the backslash and the hexadecimal digits themselves (for example: \x20 or \x4A).

			Several string literals can be concatenated to form a single string literal simply by separating them by one
			or more blank spaces, including tabs, newlines, and other valid blank characters.
			For example:
			...
			that merges both that line and the next into a single line.
			...
			where sequence is any sequence of characters (including an empty sequence).
			The content of the string is what lies inside the parenthesis, ignoring the delimiting sequence itself.
			For example:
			...
			The R prefix can be combined with any other prefixes, such as u, L or u8.
		...
		*/
		{
			...
			*/
			...
			double dVal = 3.14159L;	// long double
