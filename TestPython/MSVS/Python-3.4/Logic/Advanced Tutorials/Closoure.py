...
# Its very important to note that the nested functions can access the variables of the enclosing scope.
# However, atleast in python, they are only readonly.
...
#         nonlocal number
#         number=3
#         print (number) printer() print (number) print_msg(9) 

# Without the nonlocal keyword, the output would be 3, however, with its usage, we get 3,
# that is the value of the "number" variable gets modified. 9 3

# Now, how about we return the function object rather than calling the nested function within.
# (Remember that functions are even objects(Its Python))
...
# And we call the function as follows: >>>fun2 = transmit_to_space("Burn the Sun!") >>>fun2() >>>Burn the Sun!

# Even though the execution of the "transmit_to_space()" was completed, the message was rather preserved.
# This technique by which the data is attached to some code even after
# end of those other original functions is called as closures in python
...
# That is using closures, one could make functions to create multiply_with_5()
