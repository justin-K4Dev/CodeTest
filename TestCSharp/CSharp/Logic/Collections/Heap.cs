    ...
    public class Heap
    {
        ...
        static void binary_heap_use()
        {
            /*
                ...
                이런 Remove() 메서드를 여러 번 호출하면,
                완전히 정렬(Sort)되어 있지 않은 자료 구조로부터 최대 (혹은 최소)의 Top N 개 데이타를 가져오는 효과를 갖게 된다.
                Binary Heap 클래스를 구현하기 위해 Binary Tree를 사용할 수도 있고,
                동적 배열을 사용하여 이진트리를 구현하는 방식으로 아래 예제와 같이 .NET의 동적배열인 List 템플릿 클래스를 활용할 수도 있다.
                ...
                차이는 Add, Remove 메서드의 키값 비교에서 부등호를 서로 반대로 하는 것 정도이다.
                (참조: 아래는 Max Heap의 예만 표현하였는데,
                IComparer를 입력 받아 한 클래스에서 Max Heap/Min Heap을 같이 구현할 수도 있다)
            */
            {
                ...
                int value = maxHeap.RemoveOne();
                Console.WriteLine("Remove - Value:{0}", value);
