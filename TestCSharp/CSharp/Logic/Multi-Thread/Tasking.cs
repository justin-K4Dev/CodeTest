...
namespace MultiThread
{
    public class Tasking
    {
		...
		{
			/*
				UI 스레드에서 doAsyncFunc3() 호출될 경우 반드시 ConfigureAwait(false) 로 설정 해야 한다.
				UI Control 들은 오직 UI 스레드만에서 Control 들을 갱신 할 수 있다.
				UI 스레드는 기본적으로 SynchronizationContext 를 가지고 있다.
				SynchronizationContext: 다양한 동기화 모델에서 동기화 컨텍스트를 전파하는 기본 기능을 제공 해주는 class !!!

				var current_context = SynchronizationContext.Current;
				var http_response_task = http_client.GetAsync("https://www.bynder.com");
				http_response_task.ContinueWith( delegate
				{
					if (current_context == null)
					{
						return await http_resonse.Content.ReadAsStringAsync();
					}   
					else
					{
						// 아래와 같은 처리는 ConfigureAwait(false) 호출과 동일 하다 !!!
						current_context.Post( delegate {
							await http_resonse.Content.ReadAsStringAsync();
						}, null);
					}
		...
		static void Task_with_FromCurrentSynchronizationContext()
		{
			/*
                ...
                * 동기 컨텍스트 스케쥴러는 정적인 TaskScheduler.FromCurrentSynchronizationContext 메소드에서 얻을 수 있다.
			      이 스케쥴러는 Windows Forms 나 WPF 등의 GUI 애플리케이션에 주로 사용되며,
			...
			*/
			{
				...
				{
					System.Threading.Thread.Sleep(3000); //---- Do Somothing
				});
				t.ContinueWith(parent =>
				{
					// 현재 Form 객체내의 컨트롤러 객체에 접근 하여 값 갱신 !!!
