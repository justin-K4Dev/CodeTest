...
{
	/*
		LunaMeta is an enhanced it works with Lua 5.2 (up to 5.3!).
		This Library is also faster, when it comes to pushing and creating Objects from both Lua and C++.
		I also restructured LunMeta to use less memory since an Objects Methods and Properties are only stored once in a whole State.
		...
		*/
		...
		static void Register(lua_State * L, const char *namespac = NULL) {
			...
			if (namespac && strlen(namespac))
			{
				...
				lua_pushcfunction(L, &LuaMeta< T >::constructor);
			...
			lua_pushcfunction(L, &LuaMeta< T >::gc_obj);
			lua_settable(L, metatable);

			lua_pushstring(L, "__tostring");
			lua_pushcfunction(L, &LuaMeta< T >::to_string);
			lua_settable(L, metatable);

			lua_pushstring(L, "__eq");		// To be able to compare two Luna objects (not natively possible with full userdata)
			lua_pushcfunction(L, &LuaMeta< T >::equals);
			lua_settable(L, metatable);

			lua_pushstring(L, "__index");
			lua_pushcfunction(L, &LuaMeta< T >::property_getter);
			lua_settable(L, metatable);

			lua_pushstring(L, "__newindex");
			lua_pushcfunction(L, &LuaMeta< T >::property_setter);
			lua_settable(L, metatable);
